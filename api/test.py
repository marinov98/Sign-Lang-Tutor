...
class SLTTesting(unittest.TestCase):
    def test_login_good(self):
        ...
        response = tester.post(
            "api/auth/register", json={"email": "test2@gmail.com", "password": "456"}
        )
        self.assertTrue(response.status_code == 201 or response.status_code == 409)

        response = tester.post(
            "api/auth/login", json={"email": "test2@gmail.com", "password": "456"}
        )
    ...
    def test_login_bad(self):
        ...
        # wrong password
        response = tester.post(
            "api/auth/login", json={"email": "test2@gmail.com", "password": "fail"}
        )
        self.assertIn(b"Email and password do not match!", response.data)

        # wrong email
        response = tester.post(
            "api/auth/login", json={"email": "fail@gmail.com", "password": "456"}
        )
        self.assertIn(b"Email and password do not match!", response.data)
